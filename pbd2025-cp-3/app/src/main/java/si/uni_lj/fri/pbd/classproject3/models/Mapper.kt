...
object Mapper {
    ...
    fun mapRecipeDetailsToRecipeDetailsIm(isFavorite: Boolean?, dto: RecipeDetails): RecipeDetailsIM {
        return RecipeDetailsIM(
            // Use the isFavorite from the entity if not overridden,
            // otherwise use the parameter (which could be from a new check or user action)
            isFavorite = isFavorite ?: dto.isFavorite,
            idMeal = dto.idMeal,
            strMeal = dto.strMeal,
            strCategory = dto.strCategory,
            strArea = dto.strArea,
